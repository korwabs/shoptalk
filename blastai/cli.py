...
def serve(config: Optional[str], no_metrics_output: bool, server_port: int, web_port: int, component: Optional[str] = None):
    ...
    try:
        ...
        try:
            # 웹 포트와 서버 포트 명시적으로 해제
            if sys.platform == 'win32':
                # Windows 시스템
                try:
                    # 웹 포트 해제
                    result = subprocess.run(f'netstat -ano | findstr :{web_port}', shell=True, stdout=subprocess.PIPE, text=True)
                    ...
                    # 서버 포트 해제
                    result = subprocess.run(f'netstat -ano | findstr :{server_port}', shell=True, stdout=subprocess.PIPE, text=True)
                    for line in result.stdout.splitlines():
                        if 'LISTENING' in line:
                            pid = line.strip().split()[-1]
                            subprocess.run(f'taskkill /PID {pid} /F', shell=True, check=False)
                ...
                try:
                    # 웹 포트 해제
                    subprocess.run(['lsof', '-ti', f':{web_port}', '-sTCP:LISTEN'], check=False, stdout=subprocess.PIPE)
                    pids = subprocess.check_output(['lsof', '-ti', f':{web_port}', '-sTCP:LISTEN']).decode().strip().split('\n')
                    for pid in pids:
                        if pid:
                            subprocess.run(['kill', '-9', pid], check=False)
                    
                    # 서버 포트 해제
                    subprocess.run(['lsof', '-ti', f':{server_port}', '-sTCP:LISTEN'], check=False, stdout=subprocess.PIPE)
                    pids = subprocess.check_output(['lsof', '-ti', f':{server_port}', '-sTCP:LISTEN']).decode().strip().split('\n')
                    for pid in pids:
                        if pid:
                            subprocess.run(['kill', '-9', pid], check=False)
